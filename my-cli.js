  ...
  .action(async (_, options) => {
    ...
    const result = await prompts([
      ...
      },
      {
        ...
        name: "date",
        initial: `${paddingZero(current.getMonth() + 1)}-${paddingZero(current.getDate())}`
      }
    ])
    ...
    const response = await nodeFetch(`https://v0.yiketianqi.com/api?${qs.stringify(params)}`)
